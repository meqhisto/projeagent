...
## 2024-05-24 - Optimize in-memory aggregations by using targeted select
**Learning:** Over-fetching deeply nested relational data using Prisma's `include` can lead to significant database transfer latency and Node.js memory bloat when aggregating statistics in-memory.
**Action:** Replace `include: { relation: true }` with targeted `select: { ... }` blocks containing only the specific fields required for aggregation when pushing the calculation to the database is not feasible.
