...
## 2024-05-31 - Analytics Data Aggregation Without Memory Bloat
**Learning:** Using eager `include: { ... }` in Prisma `findMany` queries on analytics and statistics endpoints (e.g., `app/api/properties/stats/route.ts`) causes significant Node.js memory bloat when only specific scalar fields are needed for calculations.
**Action:** Replace `include` with targeted `select` structures that fetch only the fields strictly required by downstream calculation logic. When preserving existing `where` filters inside relations, place them at the top level of the nested relation alongside `select`.
