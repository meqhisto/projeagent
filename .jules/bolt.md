...
## 2024-09-18 - Replacing `include` with targeted `select` for complex in-memory aggregations
**Learning:** When performing complex in-memory aggregations on nested relations where database-level `groupBy` cannot be fully utilized, using `include: { relation: true }` leads to massive over-fetching of unneeded fields, increasing database transfer latency and Node.js memory bloat. Replacing it with targeted `select` blocks to fetch only the explicitly required fields drastically improves performance for endpoints computing large summaries like portfolio statistics.
**Action:** When computing multi-table statistics in-memory, always replace `include` with a targeted `select` specifying the exact fields required (e.g., status, amount, type) instead of fetching full records.
