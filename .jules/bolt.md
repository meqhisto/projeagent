...
## 2025-02-18 - Optimize in-memory aggregations by preventing overfetching of properties
**Learning:** When performing complex in-memory aggregations on nested relations where database-level `groupBy` cannot be fully utilized, using `include: { relation: true }` causes Node.js memory bloat by fetching entire object graphs.
**Action:** Always use targeted `select` blocks to fetch only the specific fields required (e.g., `id`, `status`, `amount`) to drastically reduce database transfer payload and Node.js memory bloat.
