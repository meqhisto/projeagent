...
## 2024-10-26 - [Remove unused database query]
**Learning:** An unused `monthlyTrend` query was fetching aggregated data from `prisma.transaction.groupBy` but was never returned in the API response or used in any calculations, unnecessarily consuming database resources.
**Action:** When auditing analytics endpoints, trace the usage of computed query results. Expensive database queries whose results are never included in the final response payload must be removed to save database transfer latency and load.
