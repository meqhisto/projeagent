...
## 2026-08-12 - Remove Unused Database Queries
**Learning:** Found an anti-pattern in `app/api/properties/stats/route.ts` where an expensive `prisma.transaction.groupBy()` query (`monthlyTrend`) was executed, but its result was never included in the final `NextResponse.json` payload, causing unnecessary database load and endpoint latency.
**Action:** When optimizing analytics routes, actively trace the usage of computed query results. Expensive database queries whose results are never included in the final response payload must be safely removed to eliminate pointless database load.
