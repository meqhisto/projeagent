...
## 2024-05-30 - Prevent Dead Query Execution and DB Overfetching in Analytics
**Learning:** Fetching unused variables/queries (like a `groupBy` trend query never returned in the API) and using `include` to fetch full relational objects when computing statistics in memory leads to wasted database round-trips and bloated payloads.
**Action:** Actively remove decoupled queries that are not utilized in the final response. Use `select` to target only the explicitly required scalar fields instead of `include` when fetching records to iterate over.
