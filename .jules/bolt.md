...
## 2023-10-27 - [Prisma Select Optimization]
**Learning:** Using `include` to fetch large nested relations for in-memory aggregations causes severe Node.js memory bloat and database transfer latency due to over-fetching.
**Action:** Replace `include` with targeted `select` blocks to fetch only the exact fields required for calculations when pulling large relational lists, drastically reducing the payload size while maintaining functionality.
