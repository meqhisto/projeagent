...
## 2024-05-19 - Concurrent Database Queries Optimization
**Learning:** Sequential Prisma queries in independent endpoints drastically increase the overall request latency, tying up Node.js thread resources and slowing down response times for users.
**Action:** Always inspect sequential database queries within a single API route and wrap independent ones in `Promise.all` to execute them concurrently, reducing total latency to the maximum of the longest query.
