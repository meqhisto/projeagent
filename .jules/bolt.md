...
## 2024-05-30 - Optimize properties stats route
**Learning:** Using `include: { units: true, transactions: true }` in Prisma on large unpaginated collections causes massive Node.js memory bloat by pulling unneeded fields. Furthermore, unused queries like `monthlyTrend` unnecessarily consume DB resources.
**Action:** Use targeted `select` fields to drastically reduce payload sizes when full entities are not needed, and actively prune orphaned queries whose results are omitted from final API responses.
