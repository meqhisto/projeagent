...
## 2024-05-31 - Sequential DB Queries in Autocomplete Anti-Pattern
**Learning:** Found sequential `findMany` queries (`parcel` then `customer`) in `app/api/search/route.ts`. Since autocomplete endpoints are frequently hit by user typing, sequential roundtrips artificially increase latency, harming perceived performance.
**Action:** When making multiple independent database queries (e.g., searching across multiple entities like parcels and customers), always wrap them in `await Promise.all([queryA, queryB])` to execute them concurrently and halve the database latency.
