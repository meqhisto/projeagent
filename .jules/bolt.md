...
## 2024-10-18 - Portfolio Stats Query Optimization
**Learning:** The portfolio stats endpoint fetched full properties, units, and transactions using an eager `include` and an unused `monthlyTrend` query, leading to significant DB load and memory bloat.
**Action:** Replaced `include` with targeted `select` blocks for nested relations and removed unused queries to minimize payload and memory overhead.
