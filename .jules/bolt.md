...
## 2024-05-30 - Prevent Dead Database Queries in API Responses
**Learning:** Found a completely decoupled database query (`monthlyTrend`) in `app/api/properties/stats/route.ts` that computed aggregate transaction amounts but was never included in the final `NextResponse.json` payload, needlessly wasting DB resources.
**Action:** When auditing or optimizing analytics endpoints, trace computed query results to the final payload. Completely remove decoupled queries to eliminate pointless database load, rather than executing them blindly.
