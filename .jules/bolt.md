...
## 2024-08-25 - Remove Unused Database Queries
**Learning:** Codebase analytics routes sometimes execute expensive database aggregations (like `monthlyTrend` in property stats) that are completely decoupled from the endpoint's final JSON response.
**Action:** Always verify if computed Prisma query results are actually used in the final response payload before optimizing them, and safely remove them if they are entirely unused to eliminate pointless database load.
