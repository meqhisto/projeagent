...
## 2024-05-15 - Unused Queries and Eager Loading in Stats Endpoint
**Learning:** Returning large arrays with `include` in Prisma for purely in-memory aggregations causes unnecessary node.js memory bloat. Also, unused queries left behind can cause unnecessary database load.
**Action:** Always replace `include` with targeted `select` blocks when fetching related records solely for aggregate calculations (e.g., status, amounts), and diligently remove dead code/queries to optimize performance.
