...
## 2025-02-18 - Avoid Memory Bloat with Concurrent Database Aggregations
**Learning:** In analytics/stats routes, using `findMany` with `include` to fetch large datasets (properties, units, transactions) into memory for basic counts and sums causes severe Node.js memory bloat and transfer latency. Additionally, replacing multiple queries without careful mapping can fail TypeScript type checking and break response contract.
**Action:** When calculating statistics across multiple related tables, replace eager `findMany` queries with targeted database-level aggregations (`count`, `aggregate`, `groupBy`) wrapped in `Promise.all` for concurrency. Map these database aggregation results directly to the required frontend response contract, using nullish coalescing to prevent TypeScript typing issues.
