...
## 2025-06-02 - Resolve sequential DB queries and prevent over-fetching memory bloat
**Learning:** In the `properties/stats` route, sequential execution of aggregations combined with `include: { relation: true }` caused a large memory footprint in Node.js by pulling full relational datasets before processing in JS.
**Action:** When implementing database aggregations, replace sequential queries with `Promise.all` for concurrency. Instead of using `include`, strictly use targeted `select` on nested relations to minimize database payload transfer and Node.js memory bloat.
