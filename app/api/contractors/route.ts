...
export async function GET(request: Request) {
    try {
        ...
        const contractorsWithAvg = contractors.map((c: any) => {
            const avgScore = c.ratings.length > 0
                ? c.ratings.reduce((sum: number, r: any) => sum + ((r.reliability + r.quality + r.communication + r.pricing) / 4), 0) / c.ratings.length
                : null;
