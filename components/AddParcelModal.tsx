...
import { X, Upload, Check, AlertCircle, Loader2, FileJson } from "lucide-react";
        ...
        try {
            ...
            if (data.type === "FeatureCollection" && Array.isArray(data.features)) {
                itemsToProcess = data.features.map((f: any) => {
                    ...
                    console.log("Processing feature properties:", JSON.stringify(p));
                    const extractedParcel = findProp(p, ["Parsel", "ParselNo", "parcel", "parsel", "ParselNo."]);
                    console.log("Extracted parcel:", extractedParcel);

                    // Fuzzy search for parcel if exact match failed
                    if (!extractedParcel) {
                        const fuzzyKey = Object.keys(p).find(k => k.toLowerCase().includes("parsel"));
                        if (fuzzyKey) {
                            console.log("Fuzzy match found for parcel:", fuzzyKey, p[fuzzyKey]);
                        }
                    }
            ...
            console.log("Processed items:", JSON.stringify(itemsToProcess, null, 2));
            ...
            for (const payload of itemsToProcess) {
                if (!payload.city || !payload.district || !payload.island || !payload.parsel) {
                    console.warn("Skipping invalid item:", payload);
                ...
                if (res.ok) {
                    ...
                    console.error("API Error Details:", {
                        ...
                        data: JSON.stringify(resData) // Stringify to see full content
                    });
                }
            }
            ...
            // Create notification for successful parcel addition
